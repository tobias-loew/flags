...
Furthermore, the unary minus operator is deleted for enabled unscoped enumerations. For unary plus cf. xref:unary operator+[unary operator+].
...
+
To enable the unary plus operator for an enumeration `E`, use the option  xref:BOOST_FLAGS[`BOOST_FLAGS_UNARY_PLUS`] (see also xref:unary operator+[unary operator+]).
