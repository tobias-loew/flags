...
Boost.Flags main purpose is to harden code which uses flag-like enumerations by providing type-safe bitwise operations:

* provides type-safe bitwise operations for flag-like scoped / unscoped enumerations
* turns logical errors into type errors
* disables all built-in arithmetic operators for enabled enumerations
...
* usable at compile-time / in template arguments: everything is `constexpr`
...
inaccessible for in-class `constexpr` constructions. To work around this limitation, please ensure that `BOOST_FLAGS_USING_OPERATORS()` is used before the class definition, e.g.
[source]
----
...
[source]
----
static_assert(boost::flags::enable<E>::value, "Please ensure that boost/flags.hpp is included and E is enabled correctly.");
----
...
The usage of scoped enumeration is strongly recommended, as they prohibit implicit conversion to the underlying integer type and thus provide more type safety. Further, by default scoped enumerations do not pollute the current namespace with the names of their enumerators. Nevertheless, for both kinds of enumerations Boost.Flags will detect semantical errors when using bitwise operations.
...
For unscoped enumerations the language provides built-in bitwise, logical and arithmetic operators. Hence, Boost.Flags will delete binary operators `&`, `|`, `^`, `&&`, `||`, `==`, `!=`, `+`, `-`, `*`, `/`, `%`, `<<` and `>>` if
...
Furthermore, the unary plus and unary minus operator are deleted for enabled unscoped enumerations.

For pre-increment `operator++` cf. xref:pre-increment operator++[pre-increment operator++].

## The underlying type

In most situations the usage of Boost.Flags does not depend on the underlying type of the enumeration.
Only when Boost.Flags is used with an *unscoped* enumeration `E` in connection with option xref:disable_complement[`disable_complement`] (i.e. `operator~(E) \-> E`) then the specification of the underlying type is required to prevent undefined behavior. For details xref:underlying_type[see below.]

# The long story

Why are the bits of integer types used for encoding Boolean options?
...
- simple bulk transfer in interfaces (e.g. an `uint32_t` can hold up to 32 different Boolean options)
...
* The built-in operators `&`, `|`, `^` and `~` return integer types. Assigning their results to enumeration-type variables would require explicit casts, which is another source for errors undetectable by the compiler.
...
* scoped enumerations are not subject to implicit integral promotion

[discrete]
### With Boost.Flags we can get both 
With Boost.Flags we get both support for all bitwise operators and type-safety. E.g. the following code
[source]
----
...
* the binary operators require both arguments to be of enumeration `E` resp. `complement<E>`


[#underlying_type]
## The underlying type

Here is a standard conforming decision diagram, whether the specification of the underlying type is required:
...
The `unsigned` requirement for pre C\\++20 compilers is more of a theoretical issue: it ensures that the underlying uses __two's complement__ for representation, which was always mandatory for unsigned integer types. But all major C++ compilers used two's complement also for signed integers, even before it got mandatory with C++20 (https://en.cppreference.com/w/cpp/language/types[, window=_blank]).
...
E.g. compiling the following code with Clang 16
...
BOOST_FLAGS(TriBool, BOOST_FLAGS_DISABLE_COMPLEMENT)

static constexpr TriBool t = ~false_;
----
produces the error message
[source]
----
error: integer value 4294967295 is outside the valid range of values [0, 3] for this enumeration type
----
...
* a *syntactical*: the negation flips all bits of the underlying integer. Even though `~bold` did the job in the beginning, it was different from `queen_of_hearts_name`. 
* a *semantical*: `~bold` is not a set of font-modifications: its purpose is to exclude boldness from a given set of flags.


Even though the underlying type together with its operators `&`, `|` and `~` denotes a https://en.wikipedia.org/wiki/Boolean_algebra_(structure)[Boolean algebra,window=_blank], when using it as set of flags only the operator `&` and `|` are endo-functions for the realm of flags: the operator `~` returns an element from a __complemented__ set. 

So, inside the Boolean algebra of the underlying type, there exist two sets of flags, the usual one and the set of complemented flags. Both of them resemble the semantics of https://en.wikipedia.org/wiki/Lattice_(order)[mathematical lattices, window=_blank] and are entangled by the operator `~`. 

Hence, for a Boost.Flags enabled enumeration `E` (where complement is not xref:reference.adoc#disable_complement[disabled]) the bitwise `~` operator switches between the types `E` and `complement<E>`  

* `operator~(E) \-> complement<E>`
* `operator~(complement<E>) \-> E`

[#compatible]
We call arguments for binary operators _compatible_ if the Boost.Flags provided operators are viable, i.e. each one is implicitly convertible to `E` or `complement<E>`. Additionally, for `operator==` and `operator!=` both have to be convertible to `E` or both have to be convertible to `complement<E>`. For assignment operators additional requirements apply (cf. xref:Operators[Operators]).
...
Finally, for the binary operators we have the following declarations and return types:
...
* the set of flags semantically forms a Boolean algebra, i.e. also `operator~(E) \-> E` has semantical meaning and additionally
* all bits of the underlying type correspond to flags

there exists the option xref:disable_complement[`disable_complement`] to disable the usage of `complement` and sets `operator~(E) \-> E`. 


## From flags to `bool`

One of the recommendations when hardening existing code with Boost.Flags is the use of scoped enumerations, as they do not implicitly convert to their underlying type. But this also prevents the implicit conversion in boolean contexts. Therefore Boost.Flags provides the following functions:

* `any(e) \-> bool` is equivalent to `e != E{}`
* `none(e) \-> bool` is equivalent to `e == E{}`

Furthermore, to test for intersection and entailment of flag-sets:

* `intersect(e1, e2) \-> bool` is equivalent to `e1 & e2 != E{}`
* `disjoint(e1, e2) \-> bool` is equivalent to `e1 & e2 == E{}`
* `subseteq(e1, e2) \-> bool` is equivalent to `e1 & e2 == e1`
* `subset(e1, e2) \-> bool` is equivalent to `subseteq(e1, e2) && e1 != e2`

Boost.Flags utility functions are defined in `namespace boost::flags`. To use them without qualification, please use the macro xref:BOOST_FLAGS_USING_UTILITIES[`BOOST_FLAGS_USING_UTILITIES`].

Instead of the functions `any` and `none`, we can use `operator!`

* `!e` is equivalent to `none(e)`
* `!!e` is equivalent to `any(e)`

and for `disjoint` and `intersect` we can write:

* `!(e1 & e2)` is equivalent to `disjoint(e1, e2)`
* `!!(e1 & e2)` is equivalent to `intersect(e1, e2)`

If existing code is hardened with Boost.Flags and unscoped enums are turned into scoped ones, then expressions like `e1 & e2` in Boolean contexts would have to be replaced by `any(e1 & e2)`. This is tedious and requires adding parenthesis. +
To simplify the usage Boost.Flags provides two alternatives:

[#pre_increment_operator]
* the pre-increment operator `++e`, which wraps `e` into a type which has an `explicit operator bool()` and promotes the wrapped type through further bitwise operations. +
...
+
To enable the pre-increment `operator++` for an enumeration `E`, use the option  xref:BOOST_FLAGS[`BOOST_FLAGS_PRE_INCREMENT`] (see also xref:pre-increment operator++[pre-increment operator++]).

* a pseudo operator xref:BOOST_FLAGS_AND[`BOOST_FLAGS_AND`] with the same precedence and associativity as `operator&` but returning a `bool`:
** `e1 BOOST_FLAGS_AND e2` is equivalent to `any(e1 & e2)` 

// +
// There is also the option xref:BOOST_FLAGS[`BOOST_FLAGS_LOGICAL_AND`], which enables `operator&&`, see xref:logical_operator_and[next section].
...
The logical `operator&&` and `operator||` are special among overloadable operators in C++: their built-in versions use short-circuit evaluation, but overloaded versions behave like regular functions.


[#logical_operator_and]
### Logical `and`

For integer and non Boost.Flags enabled unscoped enumeration-typed expressions `a` and `b`, both `(a) & (b)` and `(a) && (b)` are syntactically valid expressions but with different semantics

* argument evaluation: `(a) & (b)` always evaluates `b` while `(a) && (b)` uses short-circuit evaluation
* type:`(a) & (b)` is of integer type while `(a) && (b)` has type `bool`
* different results: assume `enum E { one = 1, two = 2 }` then in Boolean contexts 
** `one & two` evaluates to `false`
** `one && two` evaluates to `true`

To be on the safe side, Boost.Flags by default deletes `operator&&` for enabled enumerations. When applied to existing code, this will lead to compilation errors and enforces code revision.

Nevertheless, sometimes `operator&&` is required, e.g. when Boost.Flags is used together with other generic libraries. If for an enumeration `E`
[source]
----
...
[source]
----
    operator&&(e1, e2) -> bool { return any(e1 & e2); } 
----
Please note, that there is **no short-circuit** evaluation!
...
* `operator&&` may have been used accidentally instead of the operator `&`

So, before specifying `BOOST_FLAGS_LOGICAL_AND`, it is recommended to compile the code without it and check whether the uses of `operator&&` are correct.
====

[NOTE]
====
Short-circuit evaluation can be achieved by first converting the flag-value arguments to `bool` by using e.g. `!!(...) && !!(...)` or `any(...) && any(...)`.
====
...
For the builtin logical `operator||` apply similar semantic considerations as for `operator&&`, with the exception of results in Boolean contexts: both `(a) | (b)` and `(a) || (b)` evaluate to `true` if and only if at least one of the arguments is non-zero. But still, `operator|` lacks short-circuit evaluation.

As `operator||` is of very limited use for flag-like enumerations, Boost.Flags deletes it, when at least one argument is an enabled enumeration. So, enforcing revisiting their uses in legacy code. There is no Boost.Flags option to prevent the deleting of `operator||`.

[NOTE]
====
In cases where builtin short-circuit evaluation is needed, it can be achieved by first converting the flag-value arguments to `bool` by using e.g. `!!(...) || !!(...)` or `any(...) || any(...)`.
====
...
|`nth_bit`
|`(unsigned int n) \->` __integer type__
|`return 1 << n;`
