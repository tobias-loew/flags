...
The usage of scoped enumeration is strongly recommended, as they provide more type safety than unscoped by prohibiting implicit conversion to the underlying integer type. Further, by default they do not pollute the current namespace with the names of their enumerators.
...
* both arguments are not compatible (i.e. they are not the same enumeration or its xref:complementing_the_complement[complement], or for operators `==` and `!=` they are not the same enumeration) 

i.e. Boost.Flags will prohibit in those cases that the built-in version will be used. +

For scoped enumerations this is not necessary, as it is already prohibited by the language.
...
* For *unscoped* enumerations using the `disable_complement` option the underlying type is **always required**!

Please note, that this is not a requirement the language, when binary operators are used as endo-functions `(E,E) -> E` on an enumeration `E`.
...
[discrete]
### Switching to unscoped enumerations makes it even worse
...
Is the specification of the underlying type required?

First, a standard conforming decision diagram:
...
Ok, the `unsigned` requirement for pre C\\++20 compilers is more a theoretical issue, since all C\\++ compilers already used two's complement for signed integers, even before it got mandatory with C++20 (https://en.cppreference.com/w/cpp/language/types[, window=_blank]).
...
It may seem pedantic, but defining the underlying type in the unscoped and xref:disable_complement[`disable_complement`] case is crucial, as otherwise the `operator ~` will invoke undefined behavior. +
...
The macro `BOOST_FLAGS_USING_UTILITIES()` imports all utility functions, except `from_underlying`, into the current namespace.
