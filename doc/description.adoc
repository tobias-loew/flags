...
So, if your compiler uses signed two's complement, you can leave out the underlying type for scoped enums (https://en.cppreference.com/w/cpp/language/types).


**But, defining the underlying type for unscoped enumerations is crucial!** 

Otherwise the `operator ~` will invoke UB. Compilers can track this down, e.g. when evaluating constant expressions (cf. https://eel.is/c++draft/expr.static.cast#10[, window=_blank]). +
...
Sets of flags resemble the semantics of https://en.wikipedia.org/wiki/Lattice_(order)[mathematical lattices, window=_blank] rather than the semantics of a https://en.wikipedia.org/wiki/Boolean_algebra_(structure)[Boolean algebra].
...
there exists xref:boost_flags_disable_complement[boost_flags_disable_complement] which disables the usage of `complement` and sets `operator~(E) \-> E`. 
...
Alternatively, there is the xref:boost_flags_logical_and[`boost_flags_logical_and`], which enables `operator&&`, see xref:logically_and_and_or[next section].
...
[source]
----
constexpr bool boost_flags_logical_and(E){ return true; }
----
or `boost::flags::enable<E>` inherits from `boost::flags::logical_and`, then `operator &&` is overloaded for `E` with the semantics
[source]
----
...
* `template<typename T = int> nth_bit(unsigned int n) \-> T { return static_cast<T>(static_cast<underlying_or_identity<T>::type>(1) << n); }` +
  where type `T` can be either an enumeration or an integral type 
