...
* is non-intrusive - only a function overload or a template specializations required
...
The logical `operator &&` and `operator ||` have to handled with care, as they are the only ones to support short-circuit evaluation for the builtin versions.

Let's first take a look at the semantics of the builtin `operator &&`:
...
Since logical operators `&&` and `||` can easily get confused with their bitwise counterparts `&` and `|`, by default Boost.Flags disables `operator&&` and  `operator||`, when both arguments are implicitly convertible to integral types, to prevent accidentally calling the builtin versions.
...
[source]
----
constexpr bool boost_flags_option_logical_and(E){ return true; }
----
or `boost::flags::enable<E>` inherits from `boost::flags::option_logical_and`, then `operator &&` is overloaded for `E` with the semantics
[source]
----
...
* it evaluates differently than the builtin version and doesn't provide short-circuit evaluation
...
* `add_if_inplace(E& e, E mod, bool add)  \-> E&`   equivalent to   `e = add_if(e, mod, add)`
* `remove_if(E e, E mod, bool remove)  \-> E { return remove ? e & ~mod : e; }`
* `remove_if_inplace(E& e, E mod, bool remove)  \-> E&`   equivalent to   `e = remove_if(e, mod, remove)`
...
* `template<typename T = int> nth_bit(unsigned int n) \-> T { return 1 << n; }` +
  where type `T` can be either integral or an enumeration
