...
Even in Boolean contexts `(a) & (b)` and `(a) && (b)` return may have different results (e.g. `1 & 2 == false` and `1 && 2 == true`).
...
The logical `operator||` does not provide 
...
In Boolean contexts `(a) | (b)` and `(a) || (b)` return different results (e.g. for arguments `1` and `2`).
...
* it evaluates differently than the built-in version and doesn't provide short-circuit evaluation
* `operator&&` was accidentally used instead of the `operator&`
