...
The macro xref:BOOST_FLAGS_REL_OPS_DELETE[BOOST_FLAGS_REL_OPS_DELETE] can be used to delete relational operators.
...
[WARNING]
====
It is recommended to define `BOOST_FLAGS_REL_OPS_DELETE` for flag-like enumerations.
...
[INFO]
====
The definition of `BOOST_FLAGS_SPECIALIZE_STD_LESS(E)` will not enable range algorithms to use that total order. Here it is required to explicitly specify `boost::flags::total_order` as compare object.
====
