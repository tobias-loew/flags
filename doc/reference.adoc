...
### boost_flags_option_disable_complement

To disable the usage of the `complement` template (cf. xref:negation_operation[`operator~`]) for an enabled enumeration `E`, the function `boost_flags_option_disable_complement` has to be overloaded for `E` with:
[source]
----
constexpr bool boost_flags_option_disable_complement(E) { return true; }
----

`boost_flags_option_disable_complement` is looked up using https://en.cppreference.com/w/cpp/language/adl[ADL,window=_blank].
...
class my_class {
    ...
    enum class E:unsigned int { ... };
    friend constexpr bool boost_flags_enable(E) { return true; }
    friend constexpr bool boost_flags_option_disable_complement(E) { return true; }
    // ...
};
----

Please note, that defining `constexpr bool boost_flags_option_disable_complement(E) { return true; }` does not enable `E` for Boost.Flags. The definition of `constexpr bool boost_flags_enable(E) { return true; }` (or equivalent) is still required. 

(See also xref:option_disable_complement[`option_disable_complement`].)
...
When `boost::flags::enable` is specialized for `E` any definition of `boost_flags_enable` or `boost_flags_option_disable_complement` for `E` are ignored
...
By default the usage of `complement` is enabled but can be disabled using xref:option_disable_complement[`option_disable_complement`]. 
...
(See also xref:boost_flags_option_disable_complement[`boost_flags_option_disable_complement`].)
...
Depending on `add` either adds or removes all bits from `mod` to `e` and returns the result.
...
Similar to `add_if` but applies the `mod` to `e` and returns it as reference.
...
Depending on `remove` either removes or removes all bits from `mod` to `e` and returns the result.
...
Similar to `remove_if` but applies the `mod` to `e` and returns it as reference.
