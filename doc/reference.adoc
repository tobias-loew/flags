...
Concepts are used if available (a not disabled using xref:BOOST_FLAGS_HAS_CONCEPTS[`BOOST_FLAGS_HAS_CONCEPTS`]), 
otherwise `std::enable_if` with SFINAE, to control the viability of the library's functions and operators.
...
The class template `boost_flags_enable` is defined at global namespace to allow specialization
of it in arbitrary namespaces.
