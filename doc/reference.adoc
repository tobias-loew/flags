...
* `BOOST_FLAGS_UNARY_PLUS`            : enables unary `operator+` for `E` (see also xref:unary_plus[`unary_plus`])
...
If the specialization of `enable` inherits from `unary_plus` then unary `operator+` is enabled.
...
For an expression `e` of an enabled enumeration, the unary plus operator `+e` wraps `e` into an implementation defined type which has an `explicit operator bool()` and promotes the wrapped type through further bitwise operations. 
In boolean contexts, the expression `+e` is equivalent to `any(e)`.

(cf. xref:unary operator+[unary operator+] and xref:BOOST_FLAGS[`BOOST_FLAGS_UNARY_PLUS`])
...
* has a (non explicit) `operator E`, which returns the value `e` back
