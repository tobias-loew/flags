...
The macro `BOOST_FLAGS_USING_OPERATORS()` imports all operators from `namespace boost::flags` into the current namespace. (cf. xref:BOOST_FLAGS_NO_GLOBAL_USING[BOOST_FLAGS_NO_GLOBAL_USING]). +
This will enable ADL for Boost.Flags operators for enumerations in the current namespace. 


### BOOST_FLAGS_USING_UTILITIES

The macro `BOOST_FLAGS_USING_UTILITIES()` imports all utility functions from `namespace boost::flags` into the current namespace. (cf. xref:BOOST_FLAGS_NO_GLOBAL_USING[BOOST_FLAGS_NO_GLOBAL_USING]). +
...
### BOOST_FLAGS_NO_GLOBAL_USING

Controls the import of Boost.Flags operators and utility functions into the global namespace.

Possible values: undefined, 0 or 1

If the macro is undefined or has the value 0, then all operators and utility functions are imported into the global namespace via `using` declarations.
(cf. xref:BOOST_FLAGS_USING_ALL[BOOST_FLAGS_USING_ALL])
